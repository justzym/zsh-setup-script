...
# 安装 Oh My Zsh（使用 gitee 镜像，完全无交互）
if [ ! -d "$HOME/.oh-my-zsh" ]; then
    echo "正在通过 gitee 镜像安装 Oh My Zsh..."
    if command -v curl &> /dev/null; then
        RUNZSH=no CHSH=no sh -c "$(curl -fsSL https://gitee.com/pocmon/ohmyzsh/raw/master/tools/install.sh)" --unattended
    else
        RUNZSH=no CHSH=no sh -c "$(wget -O- https://gitee.com/pocmon/ohmyzsh/raw/master/tools/install.sh)" --unattended
    fi
else
...
# 安装 zsh-autosuggestions 插件（gitee 镜像）
if [ ! -d "$ZSH_CUSTOM/plugins/zsh-autosuggestions" ]; then
    echo "正在通过 gitee 镜像安装 zsh-autosuggestions 插件..."
    git clone https://gitee.com/chenweizhen/zsh-autosuggestions.git "$ZSH_CUSTOM/plugins/zsh-autosuggestions"
else
    echo "zsh-autosuggestions 已存在，跳过安装。"
fi

# 安装 zsh-syntax-highlighting 插件（gitee 镜像）
if [ ! -d "$ZSH_CUSTOM/plugins/zsh-syntax-highlighting" ]; then
    echo "正在通过 gitee 镜像安装 zsh-syntax-highlighting 插件..."
    git clone https://gitee.com/asddfdf/zsh-syntax-highlighting.git "$ZSH_CUSTOM/plugins/zsh-syntax-highlighting"
else
